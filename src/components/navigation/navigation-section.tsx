...
export function NavigationSection({ icon: Icon, title, path, items, onNavigate }: NavigationSectionProps) {
    ...
    <div className="space-y-1">
      <Collapsible open={isOpen} onOpenChange={setIsOpen}>
        ...
        <CollapsibleContent className="space-y-1 pl-6">
          {items.map((item) => (
            <NavigationSection key={item.title} {...item} onNavigate={onNavigate} />
          ))}
        </CollapsibleContent>
      </Collapsible>
    </div>
